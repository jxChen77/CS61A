...
def balanced(m):
    ...
    # Calculate the torques for left and right arms
    left_torque = length(left_arm) * total_weight(end(left_arm))
    right_torque = length(right_arm) * total_weight(end(right_arm))
    
    # Check if torques are equal for the current mobile
    torques_balanced = left_torque == right_torque
    

    # Recursively check if the mobiles hanging from the arms are balanced
    return torques_balanced and balanced(end(left_arm)) and balanced(end(right_arm))
