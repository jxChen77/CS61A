...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) == 0:
            ...
            power_list = list(_.power(opponent_card) for _ in player.hand)
            ind = power_list.index(max(power_list))
            print("DEBUG:", ind)
            self.attack += player.hand[ind].attack
            self.defense += player.hand[ind].defense
            best_card = player.hand.pop(ind)
        if best_card:
