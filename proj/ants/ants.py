...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        self.nearrest_place = self.place
        self.distance = 0

        for _ in range(self.lower_bound):
            if self.nearrest_place.is_hive:
                return None
            self.nearrest_place = self.nearrest_place.entrance
            self.distance+=1
        while not self.nearrest_place.bees and not self.nearrest_place.is_hive:
                if self.distance >= self.upper_bound:
                    return None
                elif self.distance < self.upper_bound:
                    self.nearrest_place = self.nearrest_place.entrance
                    self.distance+=1
        ...
        if self.nearrest_place.is_hive:
            ...
            return random_bee(self.nearrest_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            self.DamageAmount = amount + self.damage
        else:
            self.DamageAmount = amount

        for bee in list(self.place.bees):
            bee.reduce_health(self.DamageAmount)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        if not self.place.exit:
            pass
        else:
            placeNow = self.place.exit
            while placeNow.exit:
                if placeNow.ant:
                    placeNow.ant.double()
                    if placeNow.ant.is_container and placeNow.ant.ant_contained:
                        placeNow.ant.ant_contained.double() 
                placeNow = placeNow.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem 13
        if target is not None:
            target.turns_to_slow = self.slowing_turns

            def slow_action(gamestate):
                if target.turns_to_slow > 0:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        inFront = {}
        dis = 0
        while placeNow.entrance:
            for bee in placeNow.bees:
                inFront[bee] = dis
            if placeNow.ant and placeNow.ant != self:
                inFront[placeNow.ant] = dis
                if placeNow.ant.is_container:
                    inFront[placeNow.ant.ant_contains] = dis
            placeNow = placeNow.entrance
            dis+=1
        return inFront
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0, self.damage - self.distance_down*distance - self.shot_down * self.insects_shot)
        # END Problem Optional 2
