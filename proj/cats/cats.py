...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    match_select = [x for x in paragraphs if select(x)]
    if k >= len(match_select): return ''
    else: return match_select[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def inList(m):
        m_list= split(lower(remove_punctuation(m)))
        for x in subject: 
            if x in m_list: return True
        return False
    return inList 
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if max(len_typed, len_source) == 0:
        return 100.0 
    elif min(len_typed, len_source) == 0:
        return 0.0
    else:
        max_len = max(len_typed, len_source)
        total = 0
        for i in range(min(len_typed, len_source)):
            if typed_words[i] == source_words[i]:
                total += 1
        return 100 * total / len_typed
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_list = [diff_function(typed_word,y,limit) for y in word_list]
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:
        ...
        else: return diff + feline_fixes(typed[1:], source[1:], limit - diff)
    # END PROBLEM 6
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ls = [times_per_player[i][x] - times_per_player[i][x-1] for x in range(1,len(times_per_player[i]))]
        result += [ls]
